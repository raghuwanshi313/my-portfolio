...
import { getCodingProfileById, updateCodingProfile } from '../../api/codingProfiles';

interface EditCodingProfileFormProps {
    profileId: string;
    onSuccess: () => void;
}

const EditCodingProfileForm = ({ profileId, onSuccess }: EditCodingProfileFormProps) => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const profile = await getCodingProfileById(profileId);
                setFormData({
                    ...
                    question_count: profile.question_count.toString(),
                });
            } catch (error) {
                console.error('Error fetching coding profile:', error);
                alert('Failed to fetch coding profile');
            } finally {
    ...
    if (fetchLoading) {
        return (
            <div className="max-w-4xl mx-auto p-12 border border-[var(--nav-border-color)] rounded-xl shadow-lg text-center">
                <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-[var(--nav-text-hover)] mx-auto mb-4"></div>
                <p className="text-[var(--nav-text-color)]">Loading coding profile...</p>
            </div>
        );
    }
        ...
        <div className="max-w-4xl mx-auto p-6 border border-[var(--nav-border-color)] rounded-xl shadow-lg">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end">
                    ...
                    >
                        {loading ? 'Updating...' : 'Update Coding Profile'}
                    </button>
                </div>
            </form>
        </div>
...
export default EditCodingProfileForm;
